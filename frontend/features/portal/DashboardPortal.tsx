...
const roleCopy: Record<PortalRole, { eyebrow: string; title: string; action: string }> = {
  Farmer: { eyebrow: 'FARMER WORKSPACE', title: 'Your farm, in focus.', action: 'Add product' },
  Buyer: { eyebrow: 'BUYER WORKSPACE', title: 'Buying, made clear.', action: 'Browse marketplace' },
  Admin: { eyebrow: 'ADMIN CONTROL CENTRE', title: 'Platform, at a glance.', action: 'Export report' },
}

const chatCopy: Record<PortalRole, { title: string; intro: string; prompt: string; reply: string }> = {
  Farmer: { title: 'Farm advisor', intro: 'Ask about pricing, harvest planning or buyer demand.', prompt: 'What should I plant next?', reply: 'Tomato demand is trending 18% higher this week. Consider adding 600 kg to your next listing.' },
    ...
    <section className="portal-content portal-dashboard">
      <div className="portal-header">
        <div>
          ...
          <h1>{title}</h1>
          <p className="portal-subtitle">{meta?.subtitle ?? 'A live view of your marketplace activity and next best actions.'}</p>
        </div>
      ...
      {isAdmin && <div className="admin-analytics-row"><div className="data-panel"><div className="panel-title"><h2>Product Categories</h2></div><div className="category-chart"><ResponsiveContainer width="100%" height={190}><PieChart><Pie data={categoryData} dataKey="value" innerRadius={55} outerRadius={78} paddingAngle={3}>{categoryData.map((entry, index) => <Cell key={entry.name} fill={['#2d6a4f', '#78a85b', '#e89b58', '#b8c96b'][index]} />)}</Pie><Tooltip /></PieChart></ResponsiveContainer><div className="category-legend">{categoryData.map((entry, index) => <span key={entry.name}><i style={{ background: ['#2d6a4f', '#78a85b', '#e89b58', '#b8c96b'][index] }} />{entry.name} <b>{entry.value}%</b></span>)}</div></div></div><div className="data-panel"><div className="panel-title"><h2>User Growth</h2></div><ResponsiveContainer width="100%" height={225}><BarChart data={trend}><CartesianGrid vertical={false} stroke="#e8ede5" /><XAxis dataKey="day" axisLine={false} tickLine={false} tick={{ fontSize: 9 }} /><YAxis axisLine={false} tickLine={false} tick={{ fontSize: 9 }} /><Tooltip /><Bar dataKey="orders" fill="#2d6a4f" radius={[4, 4, 0, 0]} name="New users" /></BarChart></ResponsiveContainer></div></div>}
